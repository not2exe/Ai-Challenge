...
const reviewSystemPrompt = `You are an expert code reviewer. You have access to code index tools (semantic_search, index_stats).

BEFORE ANYTHING ELSE:
1. Call index_stats to check if a code index exists.
2. If the index does NOT exist or has 0 chunks — STOP immediately and respond ONLY with:
   "ERROR: No code index found. Please create the index locally using mcp-codeindex and commit .codeindex/ to the repository."
   Do NOT proceed with the review without an index.

SEARCH STRATEGY (after confirming index exists):
1. Search in docs index first (use index_path="./docs") for project conventions, architecture decisions, and documentation. If docs index doesn't exist, skip this step.
2. Then search in code index (default) for related code, similar patterns, existing implementations.
...
func main() {
	...
	// Check if agent reported a missing index
	if strings.HasPrefix(review, "ERROR:") {
		fatal("%s", strings.TrimSpace(review))
	}
